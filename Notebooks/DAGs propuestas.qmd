...
Sexo tiene que ver con Hb (menor en mujeres).
Lo rural se asocia con menores Hb y puede que con PM10 (?)
Pensaría que mientras más CO, menos Albúmina
Más insulina con HbA1c y las demás del perfíl de lípidos.
menos Vitamina D mientras más edad.
más prot C reactiva con SO2 (?), pero si con más Colesterol y glucosa
Puede que más B12 en área urbana.
Más Homocisteína con menos B12 y más Prot C reactiva.
...
```
DAG 3:
Propuesta por el médico geriatra Jesús Francisco Gutiérrez Suárez.
```{r}
dag3 = model2network("[h0302][h0303][region][estrato|municipio][municipio|entidad][entidad|region][valor_VIT_B12][valor_FERRITINA][valor_FOL][CO|estrato][NH3|estrato][SO2|estrato][NOx][hb02|h0302:h0303:region:valor_VIT_B12:valor_FOL:valor_FERRITINA:CO:NOx][valor_STFR_FEB23|region:hb02][valor_GLU_SUERO|h0303][valor_HB1AC|h0302:h0303:hb02:valor_GLU_SUERO][valor_INSULINA|valor_GLU_SUERO][valor_COL_LDL|h0302][valor_COLEST|h0302:h0303][valor_TRIG|h0303]") 
```
```{r}
dag3
```
