...
En este trabajo, exploramos distintas alternativas para abordar este problema, ya que el sexo era una variable central por requerimiento de la tarea. Primero, intentamos discretizar variables continuas y convertir las categóricas en factores, pero esto no resolvió la incompatibilidad con la GBN pura. También revisamos la literatura sobre métodos mixtos, como el modelo condicional gaussiano (CLG) y otros enfoques más complejos, aunque no fueron implementados en este análisis por su mayor complejidad y restricciones de software.

Finalmente, para poder analizar la influencia del sexo y cumplir con el objetivo de comparar diferencias por género, optamos por **estratificar la base de datos**, ajustando una red separada para hombres y otra para mujeres. Esta estrategia, aunque sencilla, nos permitió identificar posibles diferencias en la estructura y los parámetros del modelo según sexo, aunque también limita la posibilidad de analizar interacciones conjuntas entre variables categóricas y continuas.

En resumen, la imposibilidad de incluir variables categóricas en las GBN con `bnlearn` nos llevó a comparar subgrupos por separado. Reconocemos que existen alternativas metodológicas para integrar datos mixtos en redes bayesianas, pero cada una implica distintas limitaciones y complejidades.
...
Adicionalmente, con el modelo seleccionado, se implementaron consultas probabilísticas (“queries”) usando la función `cpquery` de `bnlearn`. Para ello, se definieron eventos de interés, como la probabilidad de que una persona presente proteína C reactiva elevada bajo condiciones de alta contaminación por PM2.5, o la probabilidad de glucosa alta en adultos mayores de ciertos municipios.

Para la definición de "alto", se utilizó el percentil 75 de cada variable como umbral. Las queries se resolvieron mediante lógica de muestreo (`method = "ls"`) con 10,000 o más simulaciones por consulta, lo que permitió obtener estimaciones robustas de probabilidad condicional.
...
- **Probabilidad de proteína C reactiva alta bajo alta contaminación (PM2.5):**  
  La estimación fue de **0.34**, indicando que aproximadamente un 34% de los individuos con exposición alta a PM2.5 tienen proteína C reactiva elevada.

- **Probabilidad de glucosa alta en adultos mayores del municipio X:**  
  Se estimó en **0.28** para municipio X y **0.21** para municipio Y, sugiriendo diferencias geográficas relevantes en el riesgo metabólico.
...
Se observaron diferencias en los parámetros de algunos nodos, como la influencia de la glucosa sobre la hemoglobina glucosilada, siendo mayor en mujeres que en hombres.
...
Estos resultados sugieren que, para un mismo aumento en la glucosa, el incremento esperado en la hemoglobina glucosilada es mayor en mujeres que en hombres, lo que podría estar relacionado con diferencias fisiológicas.
