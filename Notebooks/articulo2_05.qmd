...
- Se establecieron las condiciones de **evidencia** (ej. exposición a contaminantes como PM₂.₅, NOx o SO₂, o pertenecer a un grupo de edad).  
...
Respetamos la estructura causal de la DAG y sustituimos, nodo por nodo, los modelos lineales por **modelos aditivos generalizados (GAM)** usando *mgcv*. Para cada relación padre → hijo se especificó una función suave `s(·)` que permite no linealidades. Por ejemplo:
...
Para evaluar el ajuste global de la red, agregamos los criterios de información **nodo a nodo** y reportamos:

$$
\text{BIC}_{\text{bnlearn}} \;=\; \ell(\hat{\theta}; \mathbf{x}) \;-\; \frac{k}{2}\log(n),
$$

Con esta convención, valores **más altos** indican mejor ajuste penalizado (equivalente a menores AIC/BIC totales).

### 4.2. Resultado principal  
El ajuste no paramétrico obtuvo:

| Modelo                         | \(\text{Score}_{\text{BIC}}\) |
|--------------------------------|-------------------------------|
...
$$
\mathbb{P}\!\left(\text{proteina_cr}>0.001 \;\middle|\; PM_{2.5}<=155\ \mu\mathrm{g/m}^3\right) = 0.8228418
$$
